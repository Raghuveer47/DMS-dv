...
module.exports = {
  ...
  async signup(req, res) {
    try {
      ...
      const verificationLink = `http://${FRONTEND_URL}/auth/verify/${verificationToken}`;
      let mailOptions = {
  ...
  async resendVerificationLink(req, res) {
    try {
      ...
      const verificationLink = `http://localhost:4200/auth/verify/${verificationToken}`;
      let mailOptions = {
        from: process.env.SMTP_USER,
        to: email,
        subject: "Account Verification",
  ...
  async resetPassword(req, res) {
    try {
      ...
      let data = {
        ...
        resetPasswordTokenExpires: Date.now() + 2 * 60 * 1000,
      };
      const resetLink = `http://localhost:4200/auth/reset-password/${resetPasswordToken}`;
      let mailOptions = {
        from: process.env.SMTP_USER,
        to: email,
