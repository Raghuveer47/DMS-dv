...
const CreateEditQuiz = ({ mode }) => {
  ...
  const { onGetQuiz, onEditQuiz, onCreateQuiz } = useContext(QuizContext);
          ...
          onGetQuiz(
            ...
            (result) => {
              ...
              if (quizData) {
                ...
                if (!availableToEveryone && batch) {
                  setSelectedBatch(batch);
                  let batchFound = batches.find((b) => (b._id = batch._id));
                  setStudents(batchFound.students);
                  let selStudents = availableTo.map((s) => s._id);
                ...
                questions.forEach((question, i) => {
                  ...
                  if (
                    ...
                    let optionsFound = options.filter(
                      (option) => option.isCorrect
                    );
  ...
  const onSubmitForm = (e) => {
    ...
    if (!availableToEveryone) {
      data.batch = selectedBatch._id;
      data.availableTo = selectedStudents;
    }
    ...
    if (mode === "edit") {
      onEditQuiz(id, data, () => {
        navigate(`/dashboard/quiz?mode=edit&id=${id}&status=updated`);
        scrollToTop();
      });
    } else {
      onCreateQuiz(data, () => {
    ...
    <section>
      ...
      >
        <Typography gutterBottom variant="h5" component="div">
          {mode === "create" ? "Add New Quiz " : "Edit Batch"}
        </Typography>
                  ...
                  <Grid item md={6}>
                    <Autocomplete
                      ...
                      onChange={(e, newValue) => {
                        ...
                        let stds = newValue?.students;
                        if (stds) {
                          setSelectedStudents(stds.map((s) => s._id));
                      ...
                      renderInput={(params) => (
                        <TextField {...params} required label="Select Batch " />
                      )}
                    />
                  </Grid>
                  <Grid item md={12}>
                    ...
                    {students && students.length > 0 && (
                      <>
                        ...
                        {students.map((s, i) => {
                          ...
                          let label = `${email} - ${firstName} ${lastName}`;
                          return (
                            <FormGroup key={i}>
                              <FormControlLabel
                                checked={_.includes(selectedStudents, _id)}
                                onChange={(e) => onClickCheckbox(_id)}
                                control={<Checkbox />}
                                label={label}
                              />
                            </FormGroup>
